...
export function initNeuralVision() {
  ...
  fileInput.addEventListener("change", (e) => {
    ...
    if (file) processImagePayload(file);
  });
  ...
  function processImagePayload(file) {
    ...
    setTimeout(() => {
      printToVisionTerminal("> Extracting Base64 visual matrix...", true);

      setTimeout(() => {
        printToVisionTerminal("> Neural Net connection established.", true);

        setTimeout(() => {
          const simulatedPoem = `In pixels bright, a world unseen,\nA ghostly spark within the machine.\nRust and wire, code and light,\nData whispers in the night.\n\nAnalysis Complete.`;

          typewriterEffect(simulatedPoem, 40);
        }, 1200);
      }, 1000);
    }, 800);
  }
  ...
  function typewriterEffect(text, speed) {
    ...
    outputTerminal.innerHTML += `<br><br><span style="color: var(--cyberpunk-primary);" id="typing-span"></span><span class="term-cursor"> _</span>`;
    ...
    function type() {
      if (i < text.length) {
        ...
        setTimeout(() => {
          printToVisionTerminal("<br>> Awaiting next visual data...", true);
        }, 1000);
      }
    }
    type();
